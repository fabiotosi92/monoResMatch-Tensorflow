...
parser.add_argument('--disp_folder',         type=str,   help='path to estimated disparities',      required=True)
...
if __name__ == '__main__':
    ...
    if args.split == 'kitti':
        ...
        for t_id in range(num_samples):
            pred_disparities.append(np.load(os.path.join(args.disp_folder, str(t_id) + '.npy')))
        
        gt_disparities = load_gt_disp_kitti(args.gt_path)
        gt_depths, pred_depths, pred_disparities_resized = convert_disps_to_depths_kitti(gt_disparities, pred_disparities, args.width)
